...
import { ArrowLeft, Upload, X, Plus, Loader2, Link as LinkIcon, FileUp } from "lucide-react";
...
export default function EditDownload() {
  ...
  // Fetch download data
  const { data: download, isLoading: loadingDownload } = useQuery({
    queryKey: ["edit-download", id],
    queryFn: async () => {
  ...
  const canEdit = download && (download.author_id === user?.id || isFundador);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const urlsToCheck: Array<[string, string]> = [
      ...
      ...(downloadType === "link" ? [[downloadUrl, "URL de download"] as [string, string]] : []),
      ...(videoUrl ? [[videoUrl, "URL do vídeo"] as [string, string]] : []),
      ...mediaUrls.map((u, i) => [u, `URL da galeria #${i + 1}`] as [string, string]),
    ];
    for (const [url, label] of urlsToCheck) {
      if (url && !isSafeHttpUrl(url)) {
        toast({
          title: "URL inválida",
          description: `${label} deve começar com http:// ou https://`,
          variant: "destructive",
        });
        return;
      }
    ...
    return (
      <Layout>
        <div className="container py-8 text-center">
          <h1 className="text-2xl font-bold mb-4">Publicação não encontrada</h1>
          <Link to="/downloads">
    ...
    return (
      <Layout>
        <div className="container py-8 text-center">
          ...
          <Link to={`/download/${id}`}>
            <Button>Voltar para a publicação</Button>
          </Link>
        </div>
      </Layout>
    );
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <RichTextEditor
                  ...
                  placeholder="Descreva sua publicação em detalhes..."
                />
              </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {downloadFile && (
                      <p className="text-sm text-muted-foreground">
                        Novo arquivo: {downloadFile.name} (
              ...
              <div className="flex gap-4">
                <Button
                  type="submit"
                  disabled={updateMutation.isPending || isUploadingFile}
                  className="gap-2"
                >
                  {updateMutation.isPending || isUploadingFile ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                    <Upload className="h-4 w-4" />
                  )}
                  {isUploadingFile ? "Enviando arquivo..." : "Salvar Alterações"}
                </Button>
                <Button
                  type="button"
                  variant="outline"
