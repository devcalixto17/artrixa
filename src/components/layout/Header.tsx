...
export const Header = () => {
  ...
  // Verifica se um caminho corresponde à página atual
  const matchPath = (p?: string | null) => {
    if (!p) return false;
  ...
  const isSubmenuActive = (subs: any[], pageSlug: string, basePath: string): boolean => {
    return (subs || []).some((sub: any) => {
      const subPath =
        pageSlug === "skins" ? `/categoria/${sub.slug}` : `${basePath}/${sub.slug}`;
      if (sub.submenus && sub.submenus.length > 0) {
        return (
          matchPath(subPath) ||
          isSubmenuActive(sub.submenus, pageSlug, `${basePath}/${sub.slug}`)
        );
      }
      return matchPath(subPath);
    });
  };
  ...
  return (
    <header className="site-header sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <span className="font-display text-3xl font-black text-primary neon-glow tracking-wider">Butterfly </span>
            <span className="font-display text-3xl font-black text-foreground tracking-wider">GM </span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navigationItems?.map((page: any) => {
              ...
              if (hasSubmenus) {
                const isActive =
                  matchPath(path) ||
                  isSubmenuActive(page.submenus, page.slug, `/${page.slug}`);
                return (
                  <DropdownMenu key={page.id}>
                    <DropdownMenuTrigger className={`px-2 py-2 text-sm font-display font-bold transition-colors tracking-wide uppercase outline-none flex items-center gap-1 group data-[state=open]:text-primary rounded-md ${isActive ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-primary"}`}>
                      {page.title}
                      ...
                      {page.submenus.map((sub: any) => {
                              ...
                              <DropdownMenuSubContent className="bg-background border-border">
                                {sub.submenus.map((nested: any) => (
                                  <DropdownMenuItem
                                    key={nested.id}
                                    onClick={() => {
                                      if (nested.external_url) {
                                        window.open(nested.external_url, "_blank", "noopener,noreferrer");
                                      } else {
                                        navigate(`/${page.slug}/${sub.slug}/${nested.slug}`);
                                      }
                                    }}
                        ...
                        return (
                          <DropdownMenuItem
                            ...
                            onClick={() => {
                              if (sub.external_url) {
                                window.open(sub.external_url, "_blank", "noopener,noreferrer");
                              } else {
                                navigate(subPath);
                              }
                            }}
              ...
              if (page.external_url) {
                return (
                  <a
                    ...
                    className="px-2 py-2 text-sm font-display font-bold text-muted-foreground hover:text-primary transition-colors tracking-wide uppercase"
                  >
                    {page.title}
                  </a>
                );
              ...
              return (
                <Link
                  key={page.id}
                  to={path}
                  className={`px-2 py-2 text-sm font-display font-bold transition-colors tracking-wide uppercase rounded-md ${matchPath(path) ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-primary"}`}
                >
                  {page.title}
                </Link>
              );
          ...
          {navigationItems?.map((page: any) => {
            ...
            const renderMobileItem = (item: any, depth = 0, currentAccumulatedPath = "") => {
              ...
              if (hasSubs) {
                const subActive =
                  matchPath(itemPath) ||
                  isSubmenuActive(item.submenus, item.slug, itemPath);
                return (
                  <div key={item.id} className="space-y-1">
                    <button
                      onClick={toggleOpen}
                      className={`w-full flex items-center justify-between px-3 py-2 text-sm font-display font-bold hover:bg-secondary/50 rounded transition-colors tracking-wide uppercase ${depth > 0 ? 'pl-6' : ''} ${subActive ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-primary"}`}
                    >
              ...
              const linkPath = item.slug === 'skins' ? `/categoria/${item.slug}` : itemPath;
              return (
                <Link
                  key={item.id}
                  to={linkPath}
                  className={`block px-3 py-2 text-sm font-display font-bold hover:bg-secondary/50 rounded transition-colors tracking-wide uppercase ${depth > 0 ? 'pl-8' : ''} ${matchPath(linkPath) ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-primary"}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {item.title || item.name}
                </Link>
